...
type AuthContext =
  | { mode: 'api_key'; keyId: string; userId: string; scopes: string[] }
  | { mode: 'jwt'; userId: string; scopes: string[] };

// Validate auth from Worker headers (dual mode)
function validateAuth(req: Request): AuthContext | null {
  ...
  if (authMode === 'jwt') {
    return { mode: 'jwt', userId, scopes };
  }

  const keyId = req.headers.get('x-funchat-api-key-id');
  if (authMode === 'api_key' && keyId) {
    return { mode: 'api_key', keyId, userId, scopes };
  }

  // Legacy: API key header present without explicit auth mode
  if (keyId) {
    return { mode: 'api_key', keyId, userId, scopes };
  }
  ...
  try {
    ...
    if (req.method === 'POST' && pathParts.includes('transfer')) {
      ...
      if (!to_user_id || !amount || !currency) {
        if (auth.mode === 'api_key') await logUsage(supabase, auth.keyId, '/transfer', 'POST', 400, Date.now() - startTime, ipAddress);
      ...
      if (!recipient) {
        if (auth.mode === 'api_key') await logUsage(supabase, auth.keyId, '/transfer', 'POST', 404, Date.now() - startTime, ipAddress);
      ...
      if (error) {
        if (auth.mode === 'api_key') await logUsage(supabase, auth.keyId, '/transfer', 'POST', 500, Date.now() - startTime, ipAddress);
        return errorResponse('DATABASE_ERROR', error.message, 500);
      }

      // Dispatch webhook
      if (auth.mode === 'api_key') {
        await dispatchWebhook(supabase, auth.keyId, 'crypto.transfer', {
      ...
      if (auth.mode === 'api_key') await logUsage(supabase, auth.keyId, '/transfer', 'POST', 201, Date.now() - startTime, ipAddress);
    ...
    if (req.method === 'GET' && pathParts.includes('history')) {
      ...
      if (error) {
        if (auth.mode === 'api_key') await logUsage(supabase, auth.keyId, '/history', 'GET', 500, Date.now() - startTime, ipAddress);
        return errorResponse('DATABASE_ERROR', error.message, 500);
      }

      if (auth.mode === 'api_key') await logUsage(supabase, auth.keyId, '/history', 'GET', 200, Date.now() - startTime, ipAddress);
    ...
    if (req.method === 'GET' && pathParts.includes('stats')) {
      ...
      if (auth.mode === 'api_key') await logUsage(supabase, auth.keyId, '/stats', 'GET', 200, Date.now() - startTime, ipAddress);
