...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST' && pathParts.includes('conversations') && !pathParts.includes('messages') && !pathParts.includes('leave')) {
      ...
      // Fetch full conversation with members and profiles
      ...
      if (fetchError) {
        console.error('[api-chat] Failed to fetch full conversation after create:', fetchError.message);
        // Still return basic conversation if fetch fails
        await logUsage(supabase, auth, '/conversations', 'POST', 201, Date.now() - startTime, ipAddress);
        return successResponse(conversation, 201);
      }

      console.log('[api-chat] Created conversation with members:', fullConversation?.id, 'members:', fullConversation?.members?.length);
      
      await logUsage(supabase, auth, '/conversations', 'POST', 201, Date.now() - startTime, ipAddress);
      return successResponse(fullConversation, 201);
    }
    ...
    if (req.method === 'GET' && pathParts.includes('messages')) {
      ...
      // 1. Query messages (without join - foreign key doesn't exist)
      ...
      if (error) {
        await logUsage(supabase, auth, `/conversations/${conversationId}/messages`, 'GET', 500, Date.now() - startTime, ipAddress);
        return errorResponse('DATABASE_ERROR', error.message, 500);
      }
      ...
      // 3. Fetch sender profiles separately
      ...
      // 4. Merge sender info into messages
      ...
      return successResponse(data, 200, { count: data.length, limit, offset });
    }
    ...
    if (req.method === 'POST' && pathParts.includes('messages') && pathParts.includes('conversations')) {
      ...
      // 1. Insert message (without join - foreign key doesn't exist)
      ...
      // 3. Merge sender info
      const data = { ...message, sender: senderProfile };
