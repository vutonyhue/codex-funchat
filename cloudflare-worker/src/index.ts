...
// NOTE: Core chat routes are proxied to Edge Functions (api-chat) and stream via SSE from this worker.
// NOTE: /v1/calls/* have been migrated to Supabase direct (useCallsApi hook)
const USER_ROUTES = [
  '/v1/me',
  '/v1/profiles',
  '/v1/users',   // getProfile, updateStatus, updateLastSeen (search migrated to Supabase direct)
  '/v1/conversations',
  '/v1/messages',
  '/v1/reactions',
  '/v1/read-receipts',
...
// Browsers/SSE commonly send these automatically; plus app-specific headers we use.
// Required by browser/SSE + app-specific headers.
// Must include: authorization, content-type, cache-control, pragma, x-requested-with
const CORS_ALLOWED_HEADERS = [
  ...
  'x-requested-with',
  'x-funchat-api-key',
  'x-funchat-widget-token',
  'x-request-id',
].join(', ');

// Must include: x-request-id
const CORS_EXPOSE_HEADERS = ['x-request-id', 'x-ratelimit-limit', 'x-ratelimit-remaining', 'x-ratelimit-reset'].join(
  ', '
);

function parseAllowedOrigins(envAllowed?: string, extra?: string[]): string[] {
  const fromEnv = (envAllowed || '')
    .split(',')
    .map(s => s.trim())
    .filter(Boolean);
...
/**
 * Origin allowlist check supporting:
 * - exact matches (e.g. https://preview.lovable.app)
 * - wildcard subdomains (*.lovableproject.com matches https://<sub>.lovableproject.com)
 * - "*" matches anything (we still echo request origin, never "*", when credentials are used)
 */
function isAllowedOrigin(origin: string, allowed: string[]): boolean {
  ...
  for (const rawPattern of allowed) {
    ...
    if (pattern.startsWith('*.')) {
      ...
      if (!domain) continue;
      // Wildcard matches subdomains only; does not match apex.
    ...
    // Fallback: allow bare hostnames (rare, but supported).
...
function corsHeaders(
  origin: string | null,
  allowed: string[],
  opts?: { preflight?: boolean; requestHeaders?: string | null }
): HeadersInit {
  if (!origin) return {};
  if (!isAllowedOrigin(origin, allowed)) {
    // Do not echo unknown origins.
    return { Vary: 'Origin' };
  }

  // For preflight, some browsers send extra headers (and can vary by platform).
  // Echo them back when present to prevent intermittent preflight failures.
  const allowHeaders =
    opts?.preflight && opts.requestHeaders ? opts.requestHeaders : CORS_ALLOWED_HEADERS;

  const headers: Record<string, string> = {
    'Access-Control-Allow-Origin': origin, // echo back exact origin (never "*") for credential safety
...
function applyCors(resp: Response, origin: string | null, allowed: string[]): Response {
  const cors = corsHeaders(origin, allowed);
  const corsEntries = Object.entries(cors);
  if (corsEntries.length === 0) return resp;

  const headers = new Headers(resp.headers);
  for (const [k, v] of corsEntries) {
    headers.set(k, v);
  }
...
function generateRequestId(): string {
  // Prefer UUID for cross-system correlation (frontend logs, worker logs, upstream logs).
  // crypto.randomUUID() is available in Workers.
  return (crypto as any).randomUUID ? (crypto as any).randomUUID() : `req_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
}
...
async function handleUserRoute(
  ...
  } else if (targetPath.startsWith('/v1/crypto')) {
    targetPath = targetPath.replace('/v1/crypto', '/api-crypto');
  ...
  // NOTE: /v1/calls has been migrated to Supabase direct (useCallsApi hook)

  // Forward request to Supabase Edge Functions
  const targetUrl = `${env.SUPABASE_FUNCTIONS_URL}${targetPath}${url.search}`;
  console.log(`[Proxy] requestId=${requestId} user route -> ${request.method} ${targetUrl}`);
  ...
  forwardHeaders.set('x-funchat-scopes', 'chat:read,chat:write,users:read,users:write,rewards:read,rewards:write,calls:read,calls:write,crypto:read,crypto:write');
  ...
  try {
    ...
    console.log(`[Proxy] requestId=${requestId} upstreamStatus=${response.status} path=${url.pathname}`);
    ...
    if (response.status >= 500) {
      const snippet = await response
        ...
        .catch(() => '');
      console.log(`[Proxy] requestId=${requestId} upstream=${response.status} snippet="${snippet}"`);
      return errorResponse('UPSTREAM_ERROR', `Upstream error (${response.status})`, response.status, requestId, origin || undefined);
    }
...
async function handleWidgetRoute(
  ...
  const targetUrl = `${env.SUPABASE_FUNCTIONS_URL}${targetPath}${url.search}`;
  console.log(`[Proxy] requestId=${requestId} widget route -> ${request.method} ${targetUrl}`);
...
async function handleApiKeyRoute(
  ...
  console.log(`[Proxy] requestId=${requestId} api-key route -> ${request.method} ${targetUrl}`);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const envAllowedOrigins = parseAllowedOrigins(env.ALLOWED_ORIGINS);
    let corsAllowedOrigins = envAllowedOrigins;

    console.log(`[Req] requestId=${requestId} ${request.method} ${url.pathname}${url.search} origin=${origin || 'null'}`);

    // Handle CORS preflight
    if (request.method === 'OPTIONS') {
      // Always answer preflight immediately.
      const reqHdrs = request.headers.get('Access-Control-Request-Headers');
      const resp = new Response(null, {
        status: 204,
        headers: corsHeaders(origin, envAllowedOrigins, { preflight: true, requestHeaders: reqHdrs }),
      });
      return applyCors(resp, origin, envAllowedOrigins);
    }
    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/v1/widget/token/validate') {
        const resp = await handleWidgetTokenValidate(request, env, requestId, origin);
        return applyCors(resp, origin, corsAllowedOrigins);
      }

      // Public availability checks (signup flows) - no JWT required.
      if (request.method === 'GET' && (url.pathname === '/v1/users/check-username' || url.pathname === '/v1/users/check-email')) {
        // Enforce origin allowlist for browser requests.
        if (origin && !isAllowedOrigin(origin, envAllowedOrigins)) {
          const resp = errorResponse('ORIGIN_NOT_ALLOWED', 'Origin not allowed', 403, requestId);
          return applyCors(resp, origin, envAllowedOrigins);
        }
        ...
        if (url.pathname === '/v1/users/check-username') {
          fnName = 'check-username';
        ...
        const targetUrl = `${env.SUPABASE_FUNCTIONS_URL}/${fnName}?${params.toString()}`;
        console.log(`[Proxy] requestId=${requestId} public -> GET ${targetUrl}`);
        ...
        if (cfcip) forwardHeaders.set('cf-connecting-ip', cfcip);

        const upstream = await fetch(targetUrl, { method: 'GET', headers: forwardHeaders });
        console.log(`[Proxy] requestId=${requestId} upstreamStatus=${upstream.status} path=${url.pathname}`);

        const responseHeaders = new Headers(upstream.headers);
        responseHeaders.set('X-Request-ID', requestId);

        const resp = new Response(upstream.body, { status: upstream.status, statusText: upstream.statusText, headers: responseHeaders });
        return applyCors(resp, origin, corsAllowedOrigins);
      }
      ...
      if (widgetToken) {
        const resp = await handleWidgetRoute(request, env, widgetToken, requestId, origin);
        return applyCors(resp, origin, corsAllowedOrigins);
      }
      ...
      if (apiKey) {
        ...
        if (!apiKey.startsWith('fc_live_') && !apiKey.startsWith('fc_test_')) {
          const resp = errorResponse('INVALID_API_KEY', 'Invalid API key format', 401, requestId);
          return applyCors(resp, origin, corsAllowedOrigins);
        }
        ...
        if (!keyData) {
          const resp = errorResponse('INVALID_API_KEY', 'Invalid API key', 401, requestId);
          return applyCors(resp, origin, corsAllowedOrigins);
        }

        // Check if key is active
        if (!keyData.is_active) {
          const resp = errorResponse('API_KEY_INACTIVE', 'API key is inactive', 403, requestId);
          return applyCors(resp, origin, corsAllowedOrigins);
        }

        // Check expiration
        if (keyData.expires_at && new Date(keyData.expires_at) < new Date()) {
          const resp = errorResponse('API_KEY_EXPIRED', 'API key has expired', 403, requestId);
          return applyCors(resp, origin, corsAllowedOrigins);
        }

        // Check origin for API-key routes (per-key allowlist + env allowlist).
        const apiKeyAllowedOrigins = parseAllowedOrigins(env.ALLOWED_ORIGINS, keyData.allowed_origins);
        corsAllowedOrigins = apiKeyAllowedOrigins;
        if (origin && !isAllowedOrigin(origin, apiKeyAllowedOrigins)) {
          const resp = errorResponse('ORIGIN_NOT_ALLOWED', 'Origin not allowed', 403, requestId);
          // Return without echoing unknown origin.
          return applyCors(resp, origin, envAllowedOrigins);
        }

        const resp = await handleApiKeyRoute(request, env, keyData, requestId, origin);
        return applyCors(resp, origin, corsAllowedOrigins);
      }
      ...
      if (isUserRoute(url.pathname)) {
        ...
        // Support ?token= query param for SSE streams (EventSource can't send headers)
        if (!authHeader) {
          const tokenParam = url.searchParams.get('token');
          if (tokenParam) {
            authHeader = `Bearer ${tokenParam}`;
            console.log(`[Auth] requestId=${requestId} Using token from query param for ${url.pathname}`);
          }
        }

        const authResult = await verifyAuthHeader(authHeader, env);
        if (!authResult) {
          const resp = errorResponse('UNAUTHORIZED', 'Invalid or expired token', 401, requestId);
          return applyCors(resp, origin, corsAllowedOrigins);
        }
        ...
        const resp = await handleUserRoute(request, env, authResult.payload.sub, requestId, origin);
        return applyCors(resp, origin, corsAllowedOrigins);
      }

      // Unknown route
      const resp = errorResponse('NOT_FOUND', 'Endpoint not found', 404, requestId);
      return applyCors(resp, origin, corsAllowedOrigins);
    } catch (e: any) {
      console.error(`[Err] requestId=${requestId} Unhandled error:`, e?.message || e);
      const resp = errorResponse('INTERNAL_ERROR', 'Unhandled worker error', 500, requestId);
      return applyCors(resp, origin, corsAllowedOrigins);
    }
  },
};
