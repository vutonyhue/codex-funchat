...
type SuccessEnvelope = { success: boolean; data?: unknown; error?: unknown; message?: unknown };
...
function unwrapEnvelope(
  ...
  for (let i = 0; i < maxDepth; i++) {
    const asOk = isEnvelope(cur) ? cur.ok : null;
    const asSuccess = isSuccessEnvelope(cur) ? (cur as SuccessEnvelope).success : null;
    const isAnyEnvelope = asOk !== null || asSuccess !== null;

    if (!isAnyEnvelope) return { ok: true, data: cur };

    const status = asOk !== null ? asOk : (asSuccess as boolean);
