...
type MobileTab = 'chat' | 'calls' | 'settings' | 'profile';

export default function Chat() {
  const navigate = useNavigate();
  const location = useLocation();
  const { user, profile, loading: authLoading, isEmailVerified } = useAuth();
  const { conversations, loading: convsLoading, createConversation, deleteConversation, getConversation, findDirectConversation } = useConversations();
  ...
  useEffect(() => {
    ...
    // Fetch conversation by ID as fallback using hook
    let cancelled = false;
    ...
    getConversation(selectedConversationId)
      .then(conv => {
        if (cancelled) return;
        if (conv) {
          console.log('[Chat] Fallback fetch thành công:', conv.id);
          setFallbackConversation(conv);
        } else {
          console.error('[Chat] Fallback fetch thất bại');
        }
      })
      .catch(err => {
        if (!cancelled) {
  ...
  }, [selectedConversationId, conversations, getConversation]);
  ...
  // Handle confirm call from right panel - Using hook functions
  const handleConfirmCall = async () => {
    ...
    try {
      // Use hook to find or create conversation
      const existingConv = await findDirectConversation(foundProfile.id);
      if (import.meta.env.DEV) {
      ...
      if (existingConv?.id) {
        ...
        // Create new conversation via hook
        const createResult = await createConversation([foundProfile.id]);
