...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRnZWFkbW1ia3Zjc2dpenNuYnBpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njc1OTgzOTcsImV4cCI6MjA4MzE3NDM5N30.WnTzQjQc_j5GIKkrJ_dlRGInB3SBFSjiOSZqxZiQXDo';
...
export function useAIChat() {
  ...
  const generateImage = useCallback(async (content: string, userMessage: AIMessage, sourceImageUrl?: string) => {
    ...
    try {
      ...
      // Call ai-image Edge Function directly
      const { data, error } = await supabase.functions.invoke('ai-image', {
        body: { prompt: content, sourceImageUrl }
      });
      ...
      const assistantMessage: AIMessage = {
        ...
        role: 'assistant',
        content: data?.text || (sourceImageUrl ? 'Đã chỉnh sửa hình ảnh theo yêu cầu.' : 'Đây là hình ảnh bạn yêu cầu.'),
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      // Call ai-chat Edge Function directly with fetch for streaming
